								 ...
								 uint32_t timeout_ms)
{
	*rxlen = 0;
	...
	int32_t c = port_getchar_timeout_ms(timeout_ms);
	if (c < 0) {
		/* end of stream */
		return -1;
	}
	...
	switch (c) {
		...
		case YM_EOT:
			...
			return 0;
		case YM_CAN:
			c = port_getchar_timeout_ms(timeout_ms);
			if (c == YM_CAN) {
				*rxlen = -1;
				/* ok */
				return 0;
	...
	*rxdata = (uint8_t)c;
	...
	/* check CRC16 match */
	uint16_t crc16_val = ymodem_crc16((const unsigned char *)(rxdata + YM_PACKET_HEADER), rx_packet_size + YM_PACKET_TRAILER);
	if (crc16_val) {
		return 1;
	}
...
YmodemErr ymodem_receive (const char *out_filename, uint32_t maxsize) {
	...
	//   file.
	if (out_filename) {
		file_result = storage_f_open(&rx_file, out_filename, FA_WRITE);
		storage_f_close(&rx_file);
						...
						default: {
							...
							if (seq_nbr != (packets_rxed & 0xff)) {
								...
								if (packets_rxed == 0) {
									...
									UINT bytes_written;
									file_result = storage_f_open(&rx_file, current_filename, FA_OPEN_APPEND | FA_WRITE);
									file_result = storage_f_write(&rx_file, rx_packet_data + YM_PACKET_HEADER, to_write, &bytes_written);
									storage_f_close(&rx_file);
									if (file_result) goto rx_err_handler;
...
static void ymodem_send_packet (uint8_t *txdata, int32_t block_nbr) {
	...
	putchar((block_nbr == 0) ? YM_SOH : YM_STX);
	/* write seq numbers */
	putchar(block_nbr & 0xFF);
	putchar(~block_nbr & 0xFF);
	...
	/* write crc16 */
	putchar((crc16_val >> 8) & 0xFF);
	putchar(crc16_val & 0xFF);
}
...
=========================================================================*/
static void ymodem_send_packet0 (const char* filename, int32_t filesize) {
	int32_t pos = 0;
...
=========================================================================*/
static void ymodem_send_data_packets (uint8_t* txdata, uint32_t txlen, uint32_t timeout_ms) {
	int32_t block_nbr = 1;
	...
	do {
		putchar(YM_EOT);
		ch = port_getchar_timeout_ms(timeout_ms);
...
=========================================================================*/
YmodemErr ymodem_send_data (uint8_t *txdata, uint32_t txsize, const char *filename) {
	YmodemErr err = 0;
	FIL file;
	FRESULT file_result;
	...
	do {
		putchar(YM_CRC);
		ch = port_getchar_timeout_ms(1000);
	} while (ch < 0);
	...
	do {
		...
		ch = port_getchar_timeout_ms(YM_PACKET_RX_TIMEOUT_MS);
		...
		if (ch == YM_ACK) {
			...
			if (ch == YM_CRC) {
				UINT bytes_written;
				file_result = storage_f_open(&file, "foo.txt", FA_WRITE | FA_CREATE_ALWAYS);
				file_result = storage_f_write(&file, "hello\n", 6, &bytes_written);
				file_result = storage_f_close(&file);
...
YmodemErr ymodem_send (const char *filename) {
	...
	file_result = storage_f_open(&file, filename, FA_READ);
	file_result = storage_f_stat(filename, &file_info);
	file_result = storage_f_read(&file, &buff, file_info.fsize, &size);
